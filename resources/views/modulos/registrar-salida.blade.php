    ...
    <nav>
        <div class="p-4 sm:ml-64">
            <div class="border-gray-200 rounded-lg dark:border-gray-700 mt-[4.5rem]">
                <div>
                    <h2 class="h-16 text-center text-3xl text-gray-800 font-extrabold">REGISTAR SU SALIDA Y ALGUNA OBSERVACIÓN</h2>
                </div>
                @if(session('message'))
                ...
                <form class="" method="POST" action="{{ route('agregar-sedes.store') }}">
                    @csrf    
                        <div class="mx-96 grid gap-6 mb-6 md:grid-cols-1">
                            <div>
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="personero_id">
                                    REGISTRAR SU SALIDA:
                                </label>
                                <input type="number" id="first_name" class="block appearance-none w-full bg-gray-200 border border-black-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder="Ingrese DNI" name="buscar_dni" required>
                            </div>
                        </div>
                </form>
                <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
                    <table id="tabla10" class="table table-striped table-bordered stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                        <h3 class="h-16 text-center text-3xl text-gray-800 font-extrabold">Tabla de Registro de Salida </h3>
                        @if(session('message'))
                            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert" id="success-message">
                                <strong class="font-bold">¡Éxito!</strong>
                                <span class="block sm:inline">{{ session('message') }}</span>
